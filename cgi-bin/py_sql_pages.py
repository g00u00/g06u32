...
words = ('http://', http_host, '/cgi-bin/py_sql_pages.py')
...
#my_host = os.environ["REMOTE_ADDR"]
...
if (function == "page"):  # отрицанием резервируем ветвление для будущих особенных страниц с уникальным содержанием
    ...
    if (int(page_id) == 11):
        ...
        #    сравнением получаем нужную запись в получаем соответствующие значения для заголовка страницы
        cur.execute(
            """SELECT `page_id`,`page_prior_navig`,`page_title`, `page_keywords`, `page_content` FROM `sql_pages`  WHERE `page_id`  =  %s""",
            (page_id))
        db_one = cur.fetchone()
