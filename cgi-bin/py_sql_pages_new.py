...
domen = "198.211.123.137:7632"  # обязательно указать свой домен
...
db = pymysql.connect(host="127.0.0.1", user="g06u32", passwd="mysql16", db="g06u32", charset="utf8",
                     use_unicode=True)  # Open database connection
...
#    сравнением получаем нужную запись в получаем соответствующие значения для заголовка страницы
cur.execute(
    """SELECT `page_id`,`page_prior_navig`,`page_title`, `page_keywords`, `page_content` FROM `sql_pages` ORDER BY `sql_pages`.`page_prior_navig` DESC """)
db_all = cur.fetchall()
...
for result in db_all:
    ...
    if (int(result[1]) != 0):  # если 0 - убираем ссылку из навигации
        if (int(result[0]) != int(page_id)):
            print('\n<li style="margin-right: 1rem;"><a href = "%s' % href_py_file,
        ...
        print("%s" % result[2], "</a>", end='') #завершаем каждую строку

print("\n</ul>\n</nav>\n")

print('\n<main>')
if (function == "page"): # создаем основное содержание  и отрицанием резервируем ветвление для будущих особенных страниц с уникальным содержанием
    cur.execute(
        """SELECT `page_id`,`page_prior_navig`,`page_title`, `page_keywords`, `page_content` FROM `sql_pages`  
            WHERE `page_id`  =  %s""", (page_id))
    db_one = cur.fetchone()
    page_content = db_one[4] # print("\nСписок из базы для основного блока - db_one:\n",db_one, "\n")
    ...
    if (int(page_id) == 2):  py_form_processing.py_form_processing(db, cur)  # обработчик строки запроса
    if (int(page_id) == 3):  py_functions.py_pavl_02(qr_string)  # для задачи или лабораторной работающей в цикле
    if (int(page_id) == 4):  py_sql_test_cgi.py_sql_test_cgi()  # изучаем CGI
    if (int(page_id) == 5):  py_matplot.py_matplot()  # создаем графики
    if (int(page_id) == 6):  py_sql_shop.py_sql_shop(cur, qr_string)  # интерфейс для магазина
    if (int(page_id) == 7):  py_form_shop.py_form_shop(db, cur)  # обработчик строки запроса
    if (int(page_id) == 8):  form_action_file.form_action_file(qr_string)  # работаем с файлами
    if (int(page_id) == 9):  form_action_db.form_action_db(qr_string, db, cur)  # работаем с базой
    if (int(page_id) == 11): form_action_file.form_action_file_bash(qr_string)  # работаем с файлами
